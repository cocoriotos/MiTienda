...
<head><!--Data Menu options to get lists from MySLQ, Developed by julian Gonzalez-->
    ...
    <script> $(document).ready( function () {
		...
		<header>
			<hr id="HR">  <!-- Header title  -->
				<center><h1><font color="#5a697f" style="font-family: arial";>Módulo de Administración de Tienda</font></h1></center>
	...
	<!--<div id="Resume" class="tabcontent">
	 ...
	 <div id="AdminProviders" class="tabcontent">
	      <ul class="tab">
                <li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'AdminAdd')">| Adding  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'AdminList')">Listing |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'AdminEdit')">Editing |</a></li>
			</ul>
	</div>
	...
	<div id="AdminList" class="tabcontent">
	      <ul class="tab">
				...
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'ListProduct')">List Product |</a></li>
	 ...
	 <div id="AdminEdit" class="tabcontent">
	      <ul class="tab">
                <li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditUsers')">| Edit Users  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditRegions')">Edit Regions  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditSubregions')">Edit Subregions  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditCountries')">Edit Countries  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditCities')">Edit Cities  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditPartners')">Edit Partners  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditPAMS')">Edit PAMs  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditPAMSDirector')">Edit PAMs Director  |</a></li>
				<li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditLOBs')">Edit LOBs  |</a></li>
			    <li><a href="javascript:void(0)" class="tablinks" onclick="openList(event, 'EditAccessRequests')">Edit Access Requests |</a></li>
			</ul>
	</div>
	...
	<script> $(document).ready( function () {
	  ...
	  <table id="autosearch7" class="display" cellspacing="0" width="100%" border="3">
                <thead>
					...
					<tbody>
                <?php $_cont=0;
					$SQLSELECT = "SELECT * FROM channel_partners_list"; /*read the table registers*/
					/*$result_set =  mysqli_query($conn, $SQLSELECT); /*matrix with all registers*/
                    /*while($row = mysqli_fetch_array($result_set))/*while there is registers in array*/
                    /*{
                    $_cont=$_cont+1;
	...
	<div id="ListProduct" class="tabcontent">
	<script> $(document).ready( function () {
        $('#autosearch11').DataTable();
       } );</script> <!-- to apply the external file style to the tables-->
	  <table id="autosearch11" class="display" cellspacing="0" width="100%" border="3">
                <thead>
                        <tr>
								<th>LOB</th>
								<th>Product</th>
					...
					<tbody>
                <?php $_cont=0;
					$SQLSELECT = "SELECT * FROM product order by lob asc"; /*read the table registers*/
					/*$result_set =  mysqli_query($conn, $SQLSELECT); /*matrix with all registers*/
                    /*while($row = mysqli_fetch_array($result_set))/*while there is registers in array*/
                    /*{
                    $_cont=$_cont+1;
	 ...
	 <form id="login" action="adduserfinal.php" method="POST" autocomplete="off"> <!-- Form to login into application with authentication in database and valid username -->
			<center>
                ...
                <select name= "hireorpartner" required> <?php $SQLSELECT = "SELECT * FROM channel_partners_list where active = 1 order by name asc"; $result_set =  mysqli_query($conn, $SQLSELECT); while ($rows = $result_set ->fetch_assoc()) { $hireorpartner = $rows['name']; echo "<option value='$hireorpartner'>$hireorpartner</option>";} ?></select> <br><br>		
	...
	<div id="AddChannelAction" class="tabcontent" >
	 <form id="login" action="addchannelactionfinal.php" method="POST" autocomplete="off"> 
			<center> <br>
			    <font color=white id="form_title"><strong>Add Channel Case Form</strong></font><br><br>	
				...
				<select name= "channelname" required> <?php $SQLSELECT = "SELECT * FROM channel_partners_list order by name asc"; $result_set =  mysqli_query($conn, $SQLSELECT); while ($rows = $result_set ->fetch_assoc()) { $channelname = $rows['name']; echo "<option value='$channelname'>$channelname</option>";} ?></select> <br><br>              
				<label id= "username1" name="partnerkind" color="white"><strong>Partner Kind</strong></label><br> 
				...
				<input id="username1" type="text" name="actions1"  placeholder="Actions taken and results" required><br> <br>
				<label id= "username1" color="white"><strong>Task Actions taken 2</strong></label><br>
				<input id="username1" type="text" name="actions2"  placeholder="Actions taken and results" required><br> <br>
